...
func (b *Writer) displayWidth(words []byte) (dw int) {
	// wordRune := []rune(string(words))
	for i := 0; i < len(words); {
		p, s := width.Lookup(words)
		// fmt.Println(string(words), "-- s:", len(wordRune))
		if s <= 0 {
			return
		}
		dw += kindWidth(p.Kind())
...
func (b *Writer) updateWidth() {
	displayWidth := b.displayWidth(b.buf[b.pos:len(b.buf)])
	if displayWidth == 0 {
		displayWidth = utf8.RuneCount(b.buf[b.pos:len(b.buf)])
	}
